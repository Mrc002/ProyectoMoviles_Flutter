...
class AjusteCurvasScreen extends StatelessWidget {
  const AjusteCurvasScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final primaryColor = const Color(0xFFFF9800); // Naranja para Ajuste de Curvas

    return DefaultTabController(
      length: 2, // Dos calculadoras
      child: Scaffold(
        backgroundColor: isDark ? const Color(0xFF0F1E2E) : const Color(0xFFEBF4FC),
        appBar: AppBar(
          title: const Text('Ajuste de Curvas', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
          ...
          bottom: const TabBar(
            ...
            tabs: [
              Tab(text: 'Regresión Lineal'),
              Tab(text: 'Interpolación (Lagrange)'),
            ],
          ),
        ),
        body: const TabBarView(
          children: [
            RegresionLinealScreen(),
            LagrangeScreen(),
          ],
        ),
  ...
  void _showAssistant(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (context) => const _MiniChatAssistantAjustes(),
    );
  }
}

// ── ASISTENTE RÁPIDO PARA AJUSTE DE CURVAS ──
class _MiniChatAssistantAjustes extends StatefulWidget {
  const _MiniChatAssistantAjustes();
  @override
  State<_MiniChatAssistantAjustes> createState() => _MiniChatAssistantAjustesState();
}

class _MiniChatAssistantAjustesState extends State<_MiniChatAssistantAjustes> {
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.insights_rounded, color: Color(0xFFFF9800)),
                const SizedBox(width: 8),
                Text("Tutor de Curvas", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isDark ? Colors.white : const Color(0xFF1A2D4A))),
                const Spacer(),
                ...
                return Align(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: msg.isUser ? const Color(0xFFFF9800) : (isDark ? const Color(0xFF234060) : const Color(0xFFEBF4FC)),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: "Pregunta sobre Mínimos Cuadrados...",
