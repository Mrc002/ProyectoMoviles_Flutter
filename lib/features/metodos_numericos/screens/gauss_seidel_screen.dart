...
class GaussSeidelScreen extends StatefulWidget {
  const GaussSeidelScreen({super.key});

  @override
  State<GaussSeidelScreen> createState() => _GaussSeidelScreenState();
}

class _GaussSeidelScreenState extends State<GaussSeidelScreen> {
  int _n = 3;
  ...
  void _inicializarMatriz() {
    _controllers = List.generate(_n, (i) => List.generate(_n + 1, (j) => TextEditingController(text: '')));
    _iteraciones.clear();
    _mensajeError = '';
    _resultadoFinal = '';
  }
  ...
  void _calcularGaussSeidel() {
    ...
    for (int i = 0; i < _n; i++) {
      if (A[i][i] == 0) {
        setState(() => _mensajeError = 'Error: Cero en la diagonal principal. Intenta reordenar las filas.');
        return;
      }
    }

    List<double> x = List.filled(_n, 0.0);
    ...
    while (error > tol && iteracion <= maxIter) {
      ...
      List<double> xViejo = List.from(x); // Guardamos la x anterior para el error

      for (int i = 0; i < _n; i++) {
        double sum = 0;
        for (int j = 0; j < _n; j++) {
          if (i != j) {
            sum += A[i][j] * x[j]; // Aquí está la magia: Usa la x actualizada al instante
          }
        }
        ...
        if (iteracion > 1 && x[i] != 0) {
          double err = ((x[i] - xViejo[i]) / x[i]).abs() * 100;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: _calcularGaussSeidel,
            ...
            child: const Text('Resolver (Gauss-Seidel)', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.white)),
          ),
          const SizedBox(height: 12),
