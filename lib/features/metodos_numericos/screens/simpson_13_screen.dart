...
class Simpson13Screen extends StatefulWidget {
  const Simpson13Screen({super.key});

  @override
  State<Simpson13Screen> createState() => _Simpson13ScreenState();
}

class _Simpson13ScreenState extends State<Simpson13Screen> {
  ...
  void _calcularSimpson13() {
    ...
    int n = int.tryParse(_nController.text) ?? 2;

    if (n <= 0 || n % 2 != 0) {
      setState(() => _mensajeError = 'Para Simpson 1/3, el número de intervalos (n) debe ser par y mayor a 0.');
      return;
    }
    ...
    try {
      ...
      for (int i = 1; i < n; i++) {
        ...
        int coef = (i % 2 == 0) ? 2 : 4;
        suma += coef * fxi;
        _tabla.add({'i': i, 'xi': xi, 'fxi': fxi, 'coef': coef});
      }
      ...
      setState(() => _resultadoFinal = 'I ≈ ${resultado.toStringAsFixed(6)}');
    } catch (e) {
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 12),
                  TextField(controller: _nController, keyboardType: TextInputType.number, style: TextStyle(color: isDark ? Colors.white : Colors.black87), decoration: InputDecoration(labelText: 'Intervalos (n debe ser PAR)', filled: true, fillColor: isDark ? const Color(0xFF0F1E2E) : const Color(0xFFF0F7FF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
                  const SizedBox(height: 16),
                  ElevatedButton(onPressed: _calcularSimpson13, style: ElevatedButton.styleFrom(backgroundColor: primaryColor, minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Calcular Integral', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white))),
                ],
              ),
            ),
          ),
                ...
                child: SingleChildScrollView(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      columns: const [DataColumn(label: Text('i', style: TextStyle(fontWeight: FontWeight.bold))), DataColumn(label: Text('x_i', style: TextStyle(fontWeight: FontWeight.bold))), DataColumn(label: Text('f(x_i)', style: TextStyle(fontWeight: FontWeight.bold))), DataColumn(label: Text('Multiplicador', style: TextStyle(fontWeight: FontWeight.bold)))],
                      rows: _tabla.map((row) => DataRow(cells: [DataCell(Text(row['i'].toString())), DataCell(Text(row['xi'].toStringAsFixed(4))), DataCell(Text(row['fxi'].toStringAsFixed(4))), DataCell(Text('x ${row['coef']}', style: const TextStyle(color: Colors.grey)))])).toList(),
                    ),
                  ),
                ),
              ),
