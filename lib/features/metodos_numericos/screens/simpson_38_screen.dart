...
class Simpson38Screen extends StatefulWidget {
  const Simpson38Screen({super.key});

  @override
  State<Simpson38Screen> createState() => _Simpson38ScreenState();
}

class _Simpson38ScreenState extends State<Simpson38Screen> {
  ...
  void _calcularSimpson38() {
    ...
    int n = int.tryParse(_nController.text) ?? 3;

    if (n <= 0 || n % 3 != 0) {
      setState(() => _mensajeError = 'Para Simpson 3/8, el número de intervalos (n) debe ser múltiplo de 3.');
      return;
    }
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 12),
                  TextField(controller: _nController, keyboardType: TextInputType.number, style: TextStyle(color: isDark ? Colors.white : Colors.black87), decoration: InputDecoration(labelText: 'Intervalos (n múltiplo de 3)', filled: true, fillColor: isDark ? const Color(0xFF0F1E2E) : const Color(0xFFF0F7FF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
                  const SizedBox(height: 16),
                  ElevatedButton(onPressed: _calcularSimpson38, style: ElevatedButton.styleFrom(backgroundColor: primaryColor, minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Calcular Integral', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white))),
                ],
              ),
            ),
          ),
